...
USE_POWERLAW_SOURCES = True # whether to assume powerlaw source SEDs or not
...
with writer.as_default():
    ...
    # Tile beam interpolation grid to get a beam grid per antenna
    tile_shape = tf.convert_to_tensor([Nants, 1, 1, 1, 1], dtype=tf.int32)
    beams = tf.tile(A, tile_shape)
    
    # Run visibility simulation
    if USE_POWERLAW_SOURCES:
        # Sources with spectral index SEDs
        
        t0 = time.time()
        vis = tv.vis_specidx(antpos=antpos, lsts=lsts, freqs=freqs, 
                             ra=ra, dec=dec, 
                             flux=flux, spectral_idx=spectral_idx, 
                             beams=beams, freq_range=freq_range)
        t1 = time.time()
    else:
        # Sources with general SEDs
        
        # Build SED tensor
        flux_sed = flux * (tf.expand_dims(freqs, 1) / freq_ref)**(spectral_idx)
        
        t0 = time.time()
        vis = tv.vis_sed(antpos=antpos, lsts=lsts, freqs=freqs, 
                         ra=ra, dec=dec, flux=flux_sed,
                         beams=beams, freq_range=freq_range)
        t1 = time.time()
